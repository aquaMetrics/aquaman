...
The goal of `aquaman` is to calculate ecology metrics and assess the mixing zone
...
IQI prediction model created by Tom Wilding (SAMS) based on training data from SEPA And
MOWI.

## Assign Taxa

Assign bacteria families based on S-16 DNA reads. This in time may provide the
input to `iqi()` function. Currently, a Qiime 2 command line script achieves this
part of the process.
