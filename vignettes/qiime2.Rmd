---
title: "How to run the Bioinformatics Pipeline"
output: 
  word_document:
  html_document: 
    toc: yes
...
1.  Check these files will be in .fastq files.
2.  Save the files to your One Drive within the 'dna-data' folder.
3.  Name the folder with the unique survey name.

### Run Assignment

1.  On Windows, open WSL ubuntu 20.04, and 'mount' (mnt), the Windows folder containing the sequence data files. For example:

    ```{bash}

    cd /mnt/c/Users/Tim.Foster/OneDrive\ -\ Scottish\ Environment\ Protection\ Agency/Reports/Fish\ Farm/operating_iqi_surveys/ScreeningTool_SequenceData/dna-data/  
    ```
    ...
    This file is also stored in the /dna-files folder.
...
Refer to the DNA SOP for current requirements on number of reads required. Assess the '...demuxed_paired_end.qzv' files using [Qiime2 Viewer](https://view.qiime2.org/) where the number of reads can be verified.
...
taxa \<- import_qiime2(path = "C:\\\\Users\\\\Tim.Foster\\\\OneDrive - Scottish Environment Protection Agency\\\\Reports\\\\Fish Farm\\\\operating_iqi_surveys\\\\ScreeningTool_SequenceData\\\\dna-data\\\\intro-to-ngs-repeat-2023\\\\taxa.csv")

```{r, eval=FALSE}

library(aquaman)
taxa <- import_qiime2(path = "C:\\Users\\Tim.Foster\\OneDrive - Scottish Environment Protection Agency\\DNA\\Results\\BaseCalls\\taxa.csv")
...
# write.csv(iqi, "C:/Users/Tim.Foster/OneDrive - Scottish Environment Protection Agency/DNA/Results/BaseCalls/iqi-basecalls.csv", row.names = FALSE)

```

Example of quick plot of area (not really required - this is example of test data for against external operators IQI scores)
...
iqi_compare <- dplyr::rename(iqi_compare,
                    ...
                    "sepa_only_rf" =  sepa_only_mowi_rf_test_predictions,
                    "sepa_screening_iqi" =  sepa_iqi_aquaman
                   )
